"""Tests for channel state around buffering-timeout failover.

A buffering timeout now requests a stream switch instead of switching directly
from the FFmpeg stats parser. The channel stays BUFFERING while the replacement
is being selected/prepared and returns to ACTIVE only after the replacement
